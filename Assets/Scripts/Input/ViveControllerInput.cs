...
public class ViveControllerInput : BaseInputModule
{
    ...
    public override void Process()
    {
		...
		if (CurrentPoint [0] != null && CurrentPoint[0].layer == 5) {
			CurrentPoint [0] = PointEvents [0].pointerCurrentRaycast.gameObject;
			GameObject newEntered = ExecuteEvents.ExecuteHierarchy (CurrentPoint [0], PointEvents [0], ExecuteEvents.pointerEnterHandler);
			if (newEntered != null) {
				//Debug.Log (CurrentPoint [0].name);
				guiSelected = true;
			} else {
				newEntered = ExecuteEvents.ExecuteHierarchy (CurrentPoint [0], PointEvents [0], ExecuteEvents.pointerExitHandler);
				guiSelected = false;
			}
