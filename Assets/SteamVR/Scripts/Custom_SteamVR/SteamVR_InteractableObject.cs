...
public class SteamVR_InteractableObject : MonoBehaviour
{
    ...
    private static GameObject lc, rc;
    ...
    protected virtual void Start()
    {
        ...
        lc = GameObject.Find("Controller (left)");
        rc = GameObject.Find("Controller (right)");
    }
    ...
    {
        //I KNOW THERE ARE ERRORS HERE, BUT DON'T COMMENT IT UNLESS YOU KNOW YOU DON'T WANT TO MOVE THE OBJECT 
        //IT IS ESSENTIAL FOR GRABBING
        //But, I think I fixed the error that was here, it was a null check
        if (grabAttatchMechanic == GrabAttatchType.Track_Object && GetComponent<Rigidbody>() != null
    ...
    private void ForceReleaseGrab()
    {
        if (grabbingObject)
        {
            grabbingObject.GetComponent<SteamVR_GrabInteractor>().ForceRelease();
        }
    }
    ...
    private void SetTrackPoint(GameObject point)
    {
        ...
        if (point.GetComponent<SteamVR_GrabInteractor>() && point.GetComponent<SteamVR_GrabInteractor>().controllerAttachPoint)
        {
            controllerPoint = point.GetComponent<SteamVR_GrabInteractor>().controllerAttachPoint.transform;
        }
